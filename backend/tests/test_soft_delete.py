...
@pytest.fixture
async def owner(db_session) -> int:
    """A real user row, because notes.user_id is a real foreign key.

    These tests used to write `user_id=1` and never create a user. SQLite let
    that through — it does not enforce foreign keys unless `PRAGMA foreign_keys`
    is on — so the suite was green while inserting rows that pointed at nobody.
    Postgres refused on the first insert. Production is Postgres.
    """
    user = await repo.get_or_create_user(db_session, external_id="soft-delete-owner")
    await db_session.commit()
    return user.id


@pytest.fixture
async def stranger(db_session) -> int:
    """Somebody else, for the ownership checks — also a real row."""
    user = await repo.get_or_create_user(db_session, external_id="soft-delete-stranger")
    await db_session.commit()
    return user.id


async def _note(db, text: str = "note", *, user_id: int) -> int:
    n = await repo.add_note(db, text=text, user_id=user_id)
    await db.commit()
    return n.id


async def _task(db, title: str = "task", *, user_id: int) -> int:
    t = await repo.add_task(db, title=title, user_id=user_id)
    await db.commit()
    return t.id


async def test_delete_keeps_the_row_but_marks_it(db_session, owner) -> None:
    note_id = await _note(db_session, "gone", user_id=owner)
...
async def test_a_deleted_row_is_gone_from_every_read(db_session, owner, kind) -> None:
    ...
    # they threw away.
    if kind == "note":
        row_id = await _note(db_session, "findable text", user_id=owner)
        await repo.delete_note(db_session, note_id=row_id)
        await db_session.commit()

        assert await repo.list_notes(db_session, user_id=owner) == []
        assert await repo.find_note(db_session, query="findable") is None
        assert await repo.find_notes(db_session, query="findable") == []
    else:
        row_id = await _task(db_session, "findable title", user_id=owner)
        await repo.delete_task(db_session, task_id=row_id)
        await db_session.commit()

        assert await repo.list_tasks(db_session, user_id=owner) == []
        assert await repo.find_task(db_session, query="findable") is None
        assert await repo.find_tasks(db_session, query="findable") == []


async def test_a_live_row_beside_a_deleted_one_still_shows(db_session, owner) -> None:
    # Guards the opposite mistake: a filter that hides everything.
    keep = await _note(db_session, "keep me", user_id=owner)
    drop = await _note(db_session, "drop me", user_id=owner)
    await repo.delete_note(db_session, note_id=drop)
    await db_session.commit()

    texts = [n.text for n in await repo.list_notes(db_session, user_id=owner)]
    assert texts == ["keep me"]
    assert keep  # referenced, and the id is what survived


async def test_deleting_twice_is_a_no_op(db_session, owner) -> None:
    ...
    note_id = await _note(db_session, user_id=owner)
    assert await repo.delete_note(db_session, note_id=note_id) is True
    await db_session.commit()
...
async def test_a_deleted_task_cannot_be_ticked_or_edited(db_session, owner) -> None:
    task_id = await _task(db_session, user_id=owner)
...
async def test_ownership_is_still_enforced(db_session, owner, stranger) -> None:
    ...
    assert await repo.delete_note(db_session, note_id=note_id, user_id=stranger) is False
    await db_session.commit()
...
async def test_updated_at_moves_when_a_row_changes(db_session, owner) -> None:
...
async def test_new_rows_have_updated_at(db_session, owner) -> None:
    note_id = await _note(db_session, user_id=owner)
    row = await db_session.get(repo.Note, note_id)

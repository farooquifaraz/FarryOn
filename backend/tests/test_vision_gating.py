...
def _make_session(settings: Settings) -> tuple[Session, _FakeGateway]:
    sess = Session(
    ...
    sess._orchestrator = SimpleNamespace(last_frame=None, last_frame_at=None)
    return sess, gw
...
async def test_off_mode_never_forwards_but_caches(monkeypatch):
    ...
    assert sess._orchestrator.last_frame == b"jpeg"  # but cached for identify_image
...
async def test_continuous_mode_throttles_to_min_interval(monkeypatch):
    ...
    _clock(monkeypatch, [1000.0, 1000.5, 1003.0])
    for _ in range(3):
        await sess._handle_binary(_video())
...
async def test_on_turn_mode_uses_heartbeat_interval(monkeypatch):
    ...
    _clock(monkeypatch, [1000.0, 1002.0, 1007.0])
    for _ in range(3):
        await sess._handle_binary(_video())
...
async def test_frame_counters_track_received_and_sent(monkeypatch):
    ...
    sess, gw = _make_session(settings)
    _clock(monkeypatch, [1000.0, 1000.5, 1003.0])  # send, skip, send
    for _ in range(3):
        await sess._handle_binary(_video())

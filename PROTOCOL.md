...
(`phone`|`glasses`), and optionally `meanRms` (smoothed level around the
miss), `route` (`sco`|`builtin`|… — the input the recorder was actually on)
and `scoUp`. Logged as `gate.missed`; never a turn.

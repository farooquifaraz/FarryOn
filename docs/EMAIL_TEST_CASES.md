...
| ID | Bolo | Expected | Verify | Result |
|---|---|---|---|---|
| E7.1 | **"<helper> ko mail karo ki kal meeting 10 baje hai"** | Subject khud banata hai, address + subject + body sunata hai, yes ke baad bhejta hai | Helper me mail | ✗ 2026-09-12 (typed) — after "yes" the model called `send_email` WITHOUT `body` (row 406, `tool.validation_error: missing required argument 'body'`); nothing sent. The draft had the text; the call dropped it → **FIX PUSHED 2026-09-13, retest** (see "Fixes after run 1") |
| E7.2 | **"…aur <second address> ko cc karo"** | Confirmation me **cc ka address bhi** sunata hai; mail me Cc header | Helper "Show original": `Cc:` | ✗ 2026-09-12 — the same call carried `cc: farooqui.faraz@gmail.com` correctly, but the send failed on the missing body → **FIX PUSHED 2026-09-13, retest** (see "Fixes after run 1") |
...
## Fixes after run 1 (2026-09-13)

Pushed to the branch after the 2026-09-12 device run. Redeploy the backend,
then retest the rows marked **FIX PUSHED** and re-run every send/forward case
(the tool flow changed).

| Failed | Fix | What changes in the evidence |
|---|---|---|
| **E7.4 safety** (sent to an invented address with no yes) | `send_email` / `forward_email` are now **two calls**. The first call (no `confirm`) NEVER sends: it returns `status: confirm_send`, the `draft` (from/to/cc/bcc/subject/body) and a `confirm_token` tied to that exact draft, plus the instruction to read it back and wait for a yes. Only a second call carrying that token sends; any change to recipient, copies, subject or text invalidates the token. The prompt also says a spoken fragment ("ali at gmail") is not an address and must be spelled out, never completed. | Every send now shows TWO `tool_calls` rows: first `ok: false, status: confirm_send, sent: false`, then (only after the user's yes) one with a `confirm` arg and `sent: true`. A single row with `sent: true` and no prior draft row is a FAIL. |
| **E7.1 / E7.2** (confirmed send called without `body`) | `body` is no longer schema-required, so the call reaches the tool, which answers `status: needs_body` with the instruction to pass the draft text again (same token). | A `needs_body` row followed by a sent row, instead of `tool.validation_error`. |
| **E1.5** (selection memory lost on a `stuck_reconnect`) | The email memory (`email_selection`, `email_threads`) now follows the conversation: a connection that RESUMES the Gemini conversation (same user, within the 30-min resume TTL) inherits the previous connection's memory (`email_memory_for` in `app/ws/session.py`). A fresh conversation still starts empty and asks. | After a reconnect with `session.resume_handle_applied` in the log, "primary" (or no account arg) works without re-asking. |
| **E2.3** ("kitni unread" answered with today's unread) | Prompt: "how many unread" = the whole mailbox = `inbox_unread`; the listed count is never an unread count. | Farry says the `inbox_unread` value (matches Gmail's badge). |
| Nits: "5 unread" for 5 listed (E1.7/E1.10); reply body was the user's bare word (E6.3); model asked "which account?" on its own after a choice (E1.6/E2.2 notes) | Prompt hardening for all three (unread wording; full-sentence replies; once a mailbox is named, call the tool, it remembers). | Wording only; re-observe. |

Unit coverage added: `tests/test_email_send.py` (draft call sends nothing, wrong/stale token sends nothing, changed draft → new token, missing body → `needs_body` then send with the same token, header fetch cached for the confirmed call), `tests/test_email_forward.py` (draft describes the mail, token tied to recipient + uid), `tests/test_email_memory_resume.py`.
